 ...
 */
public class ALSFunctionInputCodec implements Codec<ALSFunctionInput> {
  @Override
  public byte[] encode(final ALSFunctionInput obj) {
  ...
  @Override
  public ALSFunctionInput decode(final byte[] buf) {
    final Kryo kryo = new Kryo();
    kryo.register(MasterCacheKey.class);
    kryo.register(HdfsCacheKey.class);
    try (final Input input = new Input(buf)) {
      final ALSFunctionInput decoded = kryo.readObject(input, ALSFunctionInput.class);
      return decoded;
    }
  }
}
