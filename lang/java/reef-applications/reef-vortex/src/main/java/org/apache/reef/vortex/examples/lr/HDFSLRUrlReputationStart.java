...
final class HDFSLRUrlReputationStart implements VortexStart {
   ...
   */
  private void processResult(final SparseVector previousModel, final Collection<VortexFuture<PartialResult>> futures, final int iteration)
    ...
    for (final VortexFuture<PartialResult> future : futures) {
      ...
      numNegative += partialResult.getNumNegative();
      previousModel.addVector(partialResult.getPartialGradient());
    }

    final double accuracy = (double) numPositive / (numNegative + numPositive);
