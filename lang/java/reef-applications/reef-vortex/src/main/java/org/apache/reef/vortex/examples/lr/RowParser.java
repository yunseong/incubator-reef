...
import org.apache.reef.vortex.examples.lr.vector.SparseVector;
 ...
 */
public final class RowParser implements VortexParser<DataSet<LongWritable, Text>, List<Row>> {
  @Override
  public List<Row> parse(final DataSet<LongWritable, Text> dataSet) {
    final List<Row> vectorsList = new ArrayList<>();
    try {
      for (final Pair<LongWritable, Text> keyValue : dataSet) {
        ...
        for (int i = 1; i < split.length; i++) {
          ...
          indices[i-1] = index - 1; // Assume the index starts from 1 in the data.
          values[i-1] = value;
        }

        final SparseVector svector = new SparseVector(values, indices);
