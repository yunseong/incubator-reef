 ...
 */
public class InputCodec implements Codec<GradientFunctionInput> {

  @Override
  public byte[] encode(final GradientFunctionInput obj) {
  ...
  @Override
  public GradientFunctionInput decode(final byte[] buf) {
    final Kryo kryo = new Kryo();
    kryo.register(MasterCacheKey.class);
    kryo.register(HdfsCacheKey.class);
    try (final Input input = new Input(buf)) {
      final GradientFunctionInput decoded = kryo.readObject(input, GradientFunctionInput.class);
      return decoded;
    }
  }
}
